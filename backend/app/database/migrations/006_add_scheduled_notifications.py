...
def upgrade():
    ...
    with engine.connect() as conn:
        ...
        if "scheduled_time" not in columns:
            conn.execute(sa.text("ALTER TABLE todos ADD COLUMN scheduled_time TIMESTAMP WITH TIME ZONE"))
            conn.commit()
            print("Added scheduled_time column to todos")

        if "notification_sent" not in columns:
            conn.execute(sa.text("ALTER TABLE todos ADD COLUMN notification_sent BOOLEAN DEFAULT FALSE"))
            conn.commit()
            print("Added notification_sent column to todos")

        if "notified_at" not in columns:
            conn.execute(sa.text("ALTER TABLE todos ADD COLUMN notified_at TIMESTAMP WITH TIME ZONE"))
            conn.commit()
        ...
        if "ix_todos_scheduled_time" not in indexes:
            conn.execute(sa.text("CREATE INDEX ix_todos_scheduled_time ON todos (scheduled_time)"))
            conn.commit()
            print("Created index on scheduled_time")

        if "ix_todos_notification_sent" not in indexes:
            conn.execute(sa.text("CREATE INDEX ix_todos_notification_sent ON todos (notification_sent)"))
            conn.commit()
