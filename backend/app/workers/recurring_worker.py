...
from dateutil.rrule import rrule, MINUTELY, DAILY, WEEKLY, MONTHLY, YEARLY
        ...
        with Session(engine) as session:
            ...
            statement = (
                ...
                .where(
                    ...
                    (RecurringTask.next_due_date == None)  # noqa: E711
                )
...
def _generate_instance(recurring_task: RecurringTask, session: Session):
    ...
    if recurring_task.end_condition == 'after_occurrences':
        ...
        count_statement = (
            select(Todo)
            .where(Todo.recurring_task_id == recurring_task.id)
        )
    ...
    new_due_date = _calculate_next_due_date(
        recurring_task,
        tz
    )
    ...
    asyncio.run(event_service.publish_and_track(
