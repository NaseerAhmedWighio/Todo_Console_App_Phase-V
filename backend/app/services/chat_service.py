...
def _load_agents_sdk():
    ...
    try:
        from agents_sdk.todo_agent import API_PROVIDER, model_name, run_agent_with_user_message
...
class ChatService:
    ...
    async def process_message(self, user_id: str, user_message: str, conversation_id: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            return await self._process_with_agent_sdk(
                user_id=user_id, user_message=user_message, conversation_history=conversation_history
            )
    ...
    async def _process_with_agent_sdk(
        self, user_id: str, user_message: str, conversation_history: List[Dict[str, str]]
    ) -> str:
        """
        ...
        try:
            ...
            if AGENTS_SDK_AVAILABLE and _run_agent_with_user_message:
                ...
                response = await _run_agent_with_user_message(
                    user_message=user_message, user_id=user_id, conversation_history=conversation_history
                )
    ...
    async def _process_with_direct_tools(
        ...
        try:
            ...
            if "create" in message_lower and "task" in message_lower:
                ...
                # Debug logging
                logger.info(
                    f"Extracted task details: title={details['title']}, due_date={details['due_date']}, time_str={details['time_str']}, tag={details['tag_name']}"
                )
                ...
                if details["tag_name"]:
                    ...
                    tag_result = mcp.create_tag_tool(
                        name=details["tag_name"], color=details["tag_color"], user_id=user_id
                    )
                    ...
                    if tag_result.get("success") or "already exists" in tag_result.get("message", "").lower():
                        ...
                        if tag_id:
                            ...
                            assign_result = mcp.assign_tag_to_task_tool(task_id=task_id, tag_id=tag_id, user_id=user_id)
                ...
                or ("mark" in message_lower and "done" in message_lower)
                or ("mark" in message_lower and "completed" in message_lower)
            ):
                ...
                if "create" in message_lower:
                    ...
                    if "tag of " in message_lower:
                        tag_name = (
                            message_lower.split("tag of ", 1)[1].split(" with ")[0].split(" for ")[0].strip().title()
                        )
                    # Pattern 2: "create a tag called/named [name]"
                        ...
                        tag_name = (
                            message_lower.split(keyword, 1)[1].split(" with ")[0].split(" for ")[0].strip().title()
                        )
...
Current user ID: {user_id}"""
            ...
            payload = {"model": model, "messages": ai_messages, "max_tokens": 500, "temperature": 0.7}
    ...
    async def _execute_tool(self, function_name: str, function_args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "message": f"Tool execution failed: {str(e)}", "error_code": "EXECUTION_FAILED"}
    ...
    async def _get_or_create_conversation(self, user_id: str, conversation_id: Optional[str]) -> Conversation:
        """Get or create conversation"""
