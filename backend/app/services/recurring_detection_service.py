...
class RecurringDetectionService:
    ...
    PATTERNS = {
        'minutely': {
            'keywords': [
                # Time indicators
                'every minute', 'minutely', 'each minute',
                # Monitoring/checking
                'check', 'monitor', 'watch', 'track', 'poll',
                'refresh', 'update', 'sync', 'fetch',
                # Alerts
                'alert', 'notification', 'reminder', 'alarm',
                # Testing
                'test', 'ping', 'heartbeat', 'status check',
                # Data
                'log', 'record', 'capture', 'sample',
            ],
            'pattern': 'minutely',
            'interval': 1,
        ...
        'daily': {
            'keywords': [
                ...
                'exercise', 'walk', 'run', 'jog', 'gym', 'workout',
                'meditate', 'meditation', 'yoga', 'stretch',
                'read', 'reading', 'journal', 'journaling',
                'sleep', 'bedtime', 'wake up', 'morning routine',
                'night routine', 'skincare', 'brush teeth',
                'practice', 'study', 'learn', 'code', 'coding',
                'write', 'writing', 'draw', 'drawing',
                'pray', 'prayer', 'worship',
                # Health
                'medicine', 'medication', 'pills', 'vitamins',
                'supplements', 'insulin', 'blood pressure',
                # Pets
                'feed dog', 'feed cat', 'walk dog', 'pet feeding',
                # Household
                'clean', 'tidy', 'dishes', 'laundry', 'trash',
            ],
            'pattern': 'daily',
            'interval': 1,
        ...
        'weekly': {
            'keywords': [
                ...
                'every weekday', 'monday morning', 'friday evening',
                # Activities
                'meeting', 'standup', 'sync', 'review', 'planning',
                'grocery', 'shopping', 'market', 'costco', 'trader',
                'church', 'temple', 'mosque', 'sunday service',
                'date night', 'family time', 'game night',
                'clean house', 'deep clean', 'vacuum', 'mop',
                'haircut', 'hair cut', 'barber', 'salon',
                'therapy', 'counseling', 'session',
                'call mom', 'call dad', 'call parents', 'family call',
                # Work
                'sprint', 'scrum', 'retrospective', 'demo',
        ...
        'monthly': {
            'keywords': [
                # Time indicators
                'every month', 'monthly', 'beginning of month',
                'end of month', 'start of month', '1st of month',
                'bill', 'bills', 'payment', 'payments', 'pay',
                'rent', 'mortgage', 'lease',
                'subscription', 'subscriptions', 'subscribe',
                'insurance', 'premium', 'policy',
                'loan', 'loans', 'credit card', 'creditcard',
                'electricity', 'electric', 'water', 'gas', 'utilities',
                'internet', 'wifi', 'phone', 'mobile', 'cell phone',
                'streaming', 'netflix', 'spotify', 'hulu', 'disney',
                'gym membership', 'club membership',
                'salary', 'paycheck', 'pay day', 'payday',
                'invoice', 'billing', 'statement',
                'tax', 'taxes', 'vat', 'deduction',
                'donation', 'charity', 'tithe',
                'storage', 'cloud storage', 'icloud', 'dropbox',
                'software', 'saas', 'license', 'renewal',
            ],
            'pattern': 'monthly',
            'interval': 1,
    ...
    ONE_TIME_INDICATORS = [
        ...
        # Project-based
        'project', 'launch', 'release', 'deploy', 'migration',
        # Events
        'appointment', 'interview', 'exam', 'test', 'presentation',
        'concert', 'show', 'movie', 'event', 'conference',
        # Travel
        'flight', 'hotel', 'airbnb', 'ticket', 'visa', 'passport',
        # Shopping
        'laptop', 'phone', 'computer', 'camera', 'tv', 'furniture',
        'car', 'bike', 'vehicle', 'appliance',
        # Documents
        'apply', 'submit', 'register', 'enroll', 'sign up',
        # Medical
        'surgery', 'operation', 'procedure', 'vaccination',
        # Home
        'repair', 'fix', 'install', 'replace', 'renovate',
        # Career
        'resume', 'cv', 'portfolio', 'application', 'cover letter',
    ]
