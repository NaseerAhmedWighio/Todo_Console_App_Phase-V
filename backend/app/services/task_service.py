...
class TaskService:
    ...
    def get_tasks(
        ...
        if sort_by == "due_date":
            statement = statement.order_by(
                col(Todo.due_date).asc() if sort_order == "asc" else col(Todo.due_date).desc()
            )
    ...
    def _get_priority_case(self, sort_order: str):
        ...
        if sort_order == "asc":
            # Urgent first
            return col(Todo.priority).case([("urgent", 1), ("high", 2), ("medium", 3), ("low", 4)], else_=5)
        else:
            # Low first (reverse)
            return col(Todo.priority).case([("urgent", 4), ("high", 3), ("medium", 2), ("low", 1)], else_=0)
    ...
    def get_tasks_by_priority(self, user_id: uuid.UUID, priority: str) -> List[Todo]:
        ...
        """
        statement = select(Todo).where(Todo.user_id == user_id, Todo.priority == priority)
        return self.session.exec(statement).all()
