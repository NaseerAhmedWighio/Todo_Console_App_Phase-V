    ...
    if auto_recurring:
        ...
        if detection_result['is_recurring'] and detection_result['confidence'] >= 0.7:
            try:
                ...
                print(f"Auto-detected recurring task: {detection_result['reason']} (confidence: {detection_result['confidence']:.2f})")
...
def read_todos(
    ...
    sort_by: str = Query("created_at", description="Field to sort by (created_at, updated_at, due_date, priority, title)"),
    ...
    statement = sql_select(Todo).where(Todo.user_id == current_user.id).options(selectinload(Todo.task_tags)).options(selectinload(Todo.task_tags).selectinload(TaskTag.tag))
    ...
    if sort_by == "due_date":
        ...
        if sort_order == "asc":
            priority_case = col(Todo.priority).case(
...
def read_todo(todo_id: str, current_user: User = Depends(get_current_user), session: Session = Depends(get_session)) -> TodoResponse:
...
async def delete_todo(todo_id: str, current_user: User = Depends(get_current_user), session: Session = Depends(get_session)):
...
async def toggle_todo_completion(todo_id: str, current_user: User = Depends(get_current_user), session: Session = Depends(get_session)) -> TodoResponse:
...
async def detect_recurring_pattern(
    ...
    result = detection_service.detect_recurring(
        title=title,
