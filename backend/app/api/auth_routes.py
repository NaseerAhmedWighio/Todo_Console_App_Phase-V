...
from ..models.user import User, UserCreate, UserLogin, UserResponse, hash_password, verify_password, is_google_email, validate_email_format, get_utc_now
...
def register(user_create: UserCreate, session: Session = Depends(get_session)):
    ...
    try:
        ...
        access_token = create_access_token(
            data={"sub": str(db_user.id), "email": db_user.email, "verified": True},
...
def login(user_login: UserLogin, session: Session = Depends(get_session)):
    ...
    access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    access_token = create_access_token(
        data={"sub": str(user.id), "email": user.email, "verified": True},
        expires_delta=access_token_expires
    )
...
def resend_verification(email: str, session: Session = Depends(get_session)):
    ...
    try:
        ...
        return {
            "success": True,
            "message": "Verification email sent" if email_sent else "Verification email could not be sent. Please check SMTP configuration.",
