...
async def handle_job_trigger(request: Request, session: Session = Depends(get_session)):
    ...
    try:
        ...
        if job_type == "reminder":
            ...
            if reminder:
                ...
                if task:
                    ...
                    if user and user.is_email_verified:
                        # Send email reminder
                        due_date_str = (
                            task.due_date.strftime("%B %d, %Y at %I:%M %p UTC") if task.due_date else "No due date"
                        )
                        ...
                        if email_sent:
                            ...
                            reminder.message = "Reminder sent via Dapr Jobs API"
            ...
            return {"status": "SUCCESS", "message": "Reminder processed", "reminder_id": reminder_id}
            ...
            return {"status": "SUCCESS", "message": "Recurring task generation triggered", "task_id": result.id}

        else:
            logger.warning(f"Unknown job type: {job_type}")
            return {"status": "SUCCESS", "message": f"Unknown job type: {job_type}"}

    except Exception as e:
        logger.error(f"Error processing job trigger: {e}")
        # Return SUCCESS anyway to prevent Dapr from retrying
        return {"status": "SUCCESS", "message": f"Error processed: {str(e)}"}
