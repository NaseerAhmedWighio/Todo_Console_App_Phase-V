...
import { connectWebSocket, subscribeToTaskUpdates, getWebSocketStatus } from '@/services/chat';
...
const ChatInterface: React.FC = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      const unsubscribe = subscribeToTaskUpdates((data) => {
        console.log('Received task update in ChatInterface:', data);

        const updateMessage: Message = {
          id: `update-${Date.now()}`,
          role: 'assistant',
          content: `✓ Task updated: ${data.event} operation completed.`,
          timestamp: new Date(),
        };
  ...
  return (
    <div className="chat-interface" style={{ maxWidth: '600px', margin: '0 auto' }}>
      <div className="chat-header">
        <div className="chat-header-content">
          <div className="chat-header-icon">🤖</div>
          <div>
            <h2 className="chat-header-text" style={{ margin: 0, fontSize: '18px' }}>AI Task Assistant</h2>
            <p className="chat-header-subtitle">Manage your tasks with natural language</p>
          </div>
          <div className="websocket-status" style={{ 
            ...
            <span style={{ color: wsStatus === 'connected' ? '#10B981' : wsStatus === 'connecting' ? '#F59E0B' : '#EF4444' }}>
          ...
          messages.map((message) => (
            <div
              ...
              className={`chat-message ${
                message.role === 'user' ? 'chat-user-message' : 'chat-assistant-message'
              }`}
            >
